...
- Do NOT run shell `curl` — translate curl examples below to `query_api_direct(url=..., method=..., body=...)`.
- For POST endpoints (e.g., Open Targets GraphQL): use `method="POST"` and pass the JSON payload as `body=`.
- For multi-step shell workflows (pipes, variables): execute each step as a separate `query_api_direct` call, passing results between steps manually.
- Use relative file paths under `workspace/`.

# Life Sciences Publication Pipeline
...
**Search Query Construction** (build from entity labels in the KG):

| Entity Type | Query Pattern | Example |
|-------------|--------------|---------|
| Compound + Disease | `"{compound} {disease}"` | `"doxorubicin cardiotoxicity"` |
| Gene + Compound | `"{gene} {compound}"` | `"TOP2B doxorubicin"` |
| Pathway | `"{pathway_name}"` | `"NRF2 oxidative stress"` |
| Drug name | `"{drug}"` | `"dexrazoxane"` |
| Gene + Disease | `"{gene} {disease}"` | `"NFE2L2 KEAP1"` |

**Grounding Strength Classification**:

| Strength | Criteria |
|----------|----------|
| **Strong** | Dataset directly tests the mechanistic claim through perturbation (drug treatment, gene KO) with relevant readout |
| **Moderate** | Dataset profiles relevant genes/pathways but design does not directly test the specific edge |
